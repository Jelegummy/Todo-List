...
export const updateCategory = async (args: CreateCategoryArgs, id: string) => {
  ...
  const res = await fetchers.Patch(
    `${ENDPOINT}/category/internal/update/${id}`,
    {
      data: args,
      token: session?.user.accessToken,
    },
...
export const getCategories = async () => {
  ...
  const res = await fetchers.Get<Category[]>(
    `${ENDPOINT}/category/internal/categories`,
    {
      token: session?.user.accessToken,
    },
  )
...
export const getCategoryById = async (id: string) => {
  ...
  const res = await fetchers.Get<Category>(
    `${ENDPOINT}/category/internal/categories/${id}`,
    {
      token: session?.user.accessToken,
    },
  )
...
export const deleteCategory = async (id: string) => {
  ...
  const res = await fetchers.Delete(
    `${ENDPOINT}/category/internal/delete/${id}`,
    {
      token: session?.user.accessToken,
    },
  )
